...
from rules import get_possible_pawn_moves, get_possible_knight_moves, get_possible_bishop_moves, get_possible_rook_moves, get_possible_queen_moves, get_possible_king_moves, get_castling_moves
...
class Board:
    ...
    def calculate_all_moves(self):
        ...
        if not any(p.possible_moves for p in filter(lambda p: p.team == self.current_team, self.pieces)):
            king = next(filter(lambda p: p.is_king and p.team == self.current_team, self.pieces))
            self.winning_team = TeamType.DRAW if not king.is_checked else (TeamType.OPPONENT if self.current_team == TeamType.OUR else TeamType.OUR)
    ...
    def check_current_team_moves(self):
        for piece in filter(lambda p: p.team == self.current_team, self.pieces):
            ...
            for move in piece.possible_moves:
                ...
                simulated_board.pieces = [p for p in simulated_board.pieces if not p.same_position(move)]
                cloned_piece = next(p for p in simulated_board.pieces if p.same_piece_position(piece))
                cloned_piece.position = move.clone()
                cloned_king = next(p for p in simulated_board.pieces if p.is_king and p.team == simulated_board.current_team)
                for enemy in filter(lambda p: p.team != simulated_board.current_team, simulated_board.pieces):
                    enemy.possible_moves = simulated_board.get_valid_moves(enemy, simulated_board.pieces)
                    if enemy.is_pawn and any(m.x != enemy.position.x and m.same_position(cloned_king.position) for m in enemy.possible_moves):
                        piece.possible_moves = [m for m in piece.possible_moves if not m.same_position(move)]
                    elif any(m.same_position(cloned_king.position) for m in enemy.possible_moves):
                        piece.possible_moves = [m for m in piece.possible_moves if not m.same_position(move)]
            ...
            for enemy in filter(lambda p: p.team != self.current_team, self.pieces):
                ...
                if enemy.is_pawn and any(m.x != enemy.position.x and m.same_position(king.position) for m in enemy.possible_moves):
    ...
    def play_move(self, en_passant_move, valid_move, played_piece, destination):
        ...
        destination_piece = next((p for p in self.pieces if p.same_position(destination)), None)
        if played_piece.is_king and destination_piece and destination_piece.is_rook and destination_piece.team == played_piece.team:
            direction = 1 if destination_piece.position.x - played_piece.position.x > 0 else -1
            ...
            return True
        if en_passant_move:
            self.pieces = [piece for piece in self.pieces if not (piece.is_pawn and piece.same_position(Position(destination.x, destination.y - pawn_direction)))]
            played_piece.position.x = destination.x
            played_piece.position.y = destination.y
            played_piece.has_moved = True
            self.calculate_all_moves()
        elif valid_move:
            self.pieces = [piece for piece in self.pieces if not piece.same_position(destination)]
            played_piece.position.x = destination.x
            played_piece.position.y = destination.y
            played_piece.has_moved = True
            self.calculate_all_moves()
