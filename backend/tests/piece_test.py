...
def test_rook_edge_of_board():
    ...
    board = Board([rook], 0)
    moves = board.get_valid_moves(rook, board.pieces)
    assert any(Position.from_dict(move).same_position(Position(8, 1)) for move in moves)
...
def test_bishop_edge_of_board():
    ...
    moves = board.get_valid_moves(bishop, board.pieces)
    assert any(Position.from_dict(move).same_position(Position(1, 1)) for move in moves)
