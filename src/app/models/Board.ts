...
export class Board {
    ...
    checkCurrentTeamMoves() {
        ...
        for (const piece of this.pieces.filter(p => p.team === this.currentTeam)) {
            ...
            // check if some piece are checking the king
            const king_ = this.pieces.find(p => p.isKing && p.team === this.currentTeam)!;
            for (const enemy of this.pieces.filter(p => p.team !== this.currentTeam)) {
                enemy.possibleMoves = this.getValidMoves(enemy, this.pieces);

                if (enemy.isPawn) {
                    if (enemy.possibleMoves.some(m => m.x !== enemy.position.x
                        && m.samePosition(king_.position))) {
                        king_.isChecked = true;
                    }
                } else {
                    if (enemy.possibleMoves.some(m => m.samePosition(king_.position))) {
                        king_.isChecked = true;
                    }
                }
            }
